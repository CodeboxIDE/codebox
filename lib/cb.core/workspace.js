...
function Workspace(root, events, id) {
    ...
    this.usersInterval = setInterval(_.bind(this.checkUsers, this), 2*60*1000); // 5 minutes
...
Workspace.prototype.checkUsers = function() {
    ...
    _.each(this._users, function(user) {
        if (!user.isActif()) {
            console.log("remove unactif user ", user.userId);
